...
<h3 align="center">Turn raw AI capability into reliable engineering output.</h3>
...
Phase 0 — Prior Learnings  (3 matches in learnings/)
  • 2026-04-22  "Equipment alert systems benefit from priority queues over
                 flat queues — confirmed in 3 prior implementations"  conf 9
  • 2026-03-11  "Industrial timeouts default ≥30s, not 5s like web norms —
                 radio relays + PLCs need wider windows"               conf 8
  • 2026-02-04  "Isolate alert ingest from notification dispatch —
                 coupling caused cascade failures last project"        conf 9
...
Phase 2 — Identify  (Tree of Thoughts: 3 approaches scored)
  A. Polling with PostgreSQL LISTEN/NOTIFY              score 7.2
  B. WAL-streaming via pg-logical-replication           score 8.1  ✓
  C. Hybrid (poll + stream)                             score 6.8
  Selected B — strong fit per prior learnings, lower latency

Phase 3 — Synthesize
  Implementation outline + verification plan generated
  Verification layers required for Complex: 1, 2, 3, 4
...
- **OMEGA Loop** — `telemetry/sessions.jsonl` records every workflow invocation. `learnings/_index.jsonl` stores curated insights. Workflows retrieve prior learnings at Phase 0; new insights are appended at close. The store is grep-searchable from day one.
...
└────────────┘    └────────────┘    └────────────┘    └────────────┘
 ...
 + domain          HARVEST                              (queried at
                                                         Phase 0 of
                                                         next task)
```

1. **Classify** — Every task is sorted into one of 5 categories (CREATION, TRANSFORMATION, UNDERSTANDING, REPAIR, OPTIMIZATION) with a complexity tier and a domain. Confidence < 7 → AI asks before proceeding.
2. **Execute** — The right workflow runs with phase gates. Assumptions are tracked, confidence is scored on every output, and high-risk assumptions must be validated before late-phase work.
...
| **Sage** | Architecture | R-I-S-E + Tree of Thoughts | System design with trade-offs |
| **Scribe** | Documentation | HARVEST + Meta-Prompting | Tech writing for any audience |
| **Sentinel** | Security | R-I-S-E + C-A-R-E | OWASP / CVSS vulnerability work |
...
- **Security assessments** — Sentinel + R-I-S-E walks OWASP Top 10 with CVSS scoring and reproducible PoCs
...
- **Compliance-driven work** — Confidence Protocol + Assumption Tracking produce the audit trail that policy-bound teams need
...
The retrieval and write contract is in [`core/omega_retrieval.md`](core/omega_retrieval.md). Both stores stay local by default — your learnings reflect your work and aren't designed to ship.

---

## Reported results

These figures are from the framework author's deployments across industrial automation, legacy modernization, and enterprise software — directional, not independently benchmarked.
...
- [`integrations/claude-code/`](integrations/claude-code/) — Native slash commands + classifier skill →
...
- **Additional personas** — Follow the structure of `personas/atlas.md`
...
FORGE was developed by **Chase Logan** through hands-on experience deploying AI-driven engineering workflows across industrial automation and enterprise software. It distills domain-agnostic patterns for making AI a reliable engineering partner — not just a code generator.

[GitHub →](https://github.com/0xPliny) · Framework v1.1
